...
if (!empty($_GET['act']) && $_GET['act'] !== 'files') {
	...
	elseif ($_GET['act'] == 'correct_rename') {
		$files_dir = __DIR__ . DIRECTORY_SEPARATOR . ".." . DIRECTORY_SEPARATOR."files";
			//echo "files_dir: ".$files_dir. "<br>" ;
			if ($handle = opendir( $files_dir) ) {
				while (false !== ($fileName = readdir($handle))) {
					if ($fileName != "." && $fileName != ".." & $fileName != ".htaccess") {
						$original_full_file_path = $files_dir .DIRECTORY_SEPARATOR .$fileName;
						//echo "full original file path: ".$original_full_file_path. "<br>";
						$symbols_to_replace = array( "(", ")", "'", "[", "]", ",", "!", "@", "#", "$", "%", "^", "*" );
						...
						if($newName != $fileName){
							rename($original_full_file_path, $new_full_file_path);
							echo "'$fileName' was changed to '$newName'. <br>";
						}
					}
				}
				closedir($handle);
