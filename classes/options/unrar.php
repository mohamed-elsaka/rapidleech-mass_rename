                        ...
                        for ($i = 0; $i < count($_GET["files"]); $i++) {
                            ...
                            <tr align="center">
                                <td colspan="2" class="unrar-main-td">
                                    <input type="hidden" name="files[<?php echo $i; ?>]"
                                           value="<?php echo $_GET["files"][$i]; ?>"/>
                            ...
                            <tr>
                                <td colspan="2" class="unrar-main2-td">
                                    ...
                                    if ($rar->rar_return === false) {
                                        ...
                                        if ($rar_list[0] == 'PASS') {
                                            $rar_passl_needed = true;
                                            echo lang(368);
                                        } elseif ($rar_list['NEEDP'] == true) {
                                            echo lang(369);
                                        } elseif ($rar_list[0] == 'ERROR') {
                                            printf(lang(370), $rar_list[1] . ' ' . $rar_list[2]);
                                        }
                                    }
                                    ?>
                                    <input type="<?php echo ($rar_list['NEEDP'] == true) ? 'password' : 'hidden'; ?>"
                                           name="passwords[]" value="<?php echo $_GET['passwords'][$i]; ?>"/>
                                    &nbsp;
                                </td>
                            </tr>
                            <?php
                                ...
                                foreach ($rar_list as $rar_key => $rar_item) {
                                    ...
                                    <tr>
                                        <td class="unrar-lb-td">
                                            <input type="checkbox" name="filestounrar[<?php echo $i; ?>][]"
                                                   checked="checked" value="<?php echo base64_encode($rar_key); ?>"/>
                                        </td>
...
function unrar_go()
{
            ...
            <td>
                <table>
                    ...
                    for ($i = 0; $i < count($_GET["files"]); $i++) {
                        ...
                        <tr align="center">
                            <td colspan="2"
                                style="border-right:1px solid #666; border-left:1px solid #666; border-top:1px solid #666; padding:2px; background-color:#001825;"><?php printf(lang(373), basename($file["name"])); ?></td>
                        </tr>
                        <?php
                        foreach ($_GET['filestounrar'][$i] as $rar_item) {
                            ...
                            <tr>
                                ...
                                <td id="<?php echo 'unrar' . $_GET["files"][$i] . '-' . str_replace('=', '-', $rar_item); ?>"
                                    align="center"
                                    style="border-right:1px solid #666; padding:2px; background-color:#001825;"><?php echo lang(364); ?></td>
                            </tr>
                            <?php
                        }
                        ?>
    ...
    <span id="unrar_finished" style="display:none;"><a
            href="<?php echo $PHP_SELF . "?act=files"; ?>"><?php echo lang(365); ?></a><br/><br/><br/></span>
    <?php
}
    ...
    for ($i = 0; $i < count($_GET["files"]); $i++) {
        ...
        foreach ($_GET['filestounrar'][$i] as $rar_item) {
            ...
            if ($rar->rar_return === false) {
                ?>
                <script
                    type="text/javascript">rar_st('<?php echo 'unrar'.$_GET["files"][$i].'-'.str_replace('=', '-', $rar_item); ?>', '<?php echo lang(343); ?>');</script>
                <?php
            } else {
                ...
                if (strpos($rar_result, ", 'OK')") !== false) {
                    ...
                    if (!updateListInFile($list)) {
                        ?>
                        <script type="text/javascript">var tmp = document.getElementById('rar_finished');
                            tmp.innerHTML = "<?php echo lang(9); ?><br /><br />" + tmp.innerHTML</script>;
                        <?php
                    }
                }
            }
